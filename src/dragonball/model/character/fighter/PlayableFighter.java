...
public abstract class PlayableFighter extends Fighter implements PlayableCharacter {
	...
	public void addNewSuperAttack(SuperAttack e) throws MaximumAttacksLearnedException, DuplicateAttackException
	{if (getSuperAttacks().size()<=4)
	{for(int i=0;i<getSuperAttacks().size();i++)
	{	if(getSuperAttacks().get(i)==e)
			throw new DuplicateAttackException(e);}
	this.getSuperAttacks().add(e);}
	else 
	{
		throw new MaximumAttacksLearnedException();
	}
	}
	public void addNewUltimateAttack(UltimateAttack e) throws MaximumAttacksLearnedException, DuplicateAttackException
	{if (getSuperAttacks().size()<=2)
	{for(int i=0;i<getUltimateAttacks().size();i++)
		if(getUltimateAttacks().get(i)==e)
			throw new DuplicateAttackException(e) ;
	this.getUltimateAttacks().add(e);
	}
	else 
	{
		throw new MaximumAttacksLearnedException();
	...
	public String toString()
	{
		...
		return super.toString()+"\nability points = "+getAbilityPoints()+"\n Xp = "+getXp()+"\nTarget Xp = "+getTargetXp(); 
